...
export function clampStep(step) {
    return Math.round(clampNumber(step, SETTINGS_LIMITS.minStep, SETTINGS_LIMITS.maxStep, DEFAULT_SETTINGS.stepBelow100));
}
export function clampMinVolume(minVolume) {
    return Math.round(clampNumber(minVolume, SETTINGS_LIMITS.minVolume, SETTINGS_LIMITS.maxBelowNormalVolume, DEFAULT_SETTINGS.minVolume));
}
...
export function clampVolume(volume, maxVolume = DEFAULT_SETTINGS.maxVolume, minVolume = DEFAULT_SETTINGS.minVolume) {
    return Math.round(clampNumber(volume, clampMinVolume(minVolume), clampMaxVolume(maxVolume), DEFAULT_AUDIO_STATE.volume));
}
export function snapVolumeToStep(volume, settings = DEFAULT_SETTINGS) {
    ...
    if (normalizedVolume < SETTINGS_LIMITS.normalVolume) {
        ...
        const steppedValue = SETTINGS_LIMITS.normalVolume - Math.round(distance / settings.stepBelow100) * settings.stepBelow100;
    ...
    const steppedValue = SETTINGS_LIMITS.normalVolume + Math.round(distance / settings.stepAbove100) * settings.stepAbove100;
...
export function normalizeSettings(candidate = {}) {
    return {
        ...
        autoApplySavedLevels: Boolean(candidate.autoApplySavedLevels ?? DEFAULT_SETTINGS.autoApplySavedLevels),
