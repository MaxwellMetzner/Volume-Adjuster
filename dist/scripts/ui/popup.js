import { DEFAULT_AUDIO_STATE, MESSAGE_TYPES, SETTINGS_LIMITS, clampVolume, formatVolumeLabel, getSiteLabel, snapVolumeToStep } from "../shared/constants.js";
...
const elements = {
    anchorMarker: getRequiredElement(".js-anchor-marker"),
    attentionNote: getRequiredElement(".js-attention-note"),
    audibleCount: getRequiredElement(".js-audible-count"),
    audibleList: getRequiredElement(".js-audible-list"),
    emptyState: getRequiredElement(".js-empty-state"),
    monoButton: getRequiredElement(".js-mono-button"),
    muteButton: getRequiredElement(".js-mute-button"),
    openSettings: getRequiredElement(".js-open-settings"),
    siteLabel: getRequiredElement(".js-site-label"),
    tabsPanel: getRequiredElement(".js-tabs-panel"),
    template: getRequiredElement("#audibleTabTemplate"),
    tickStrip: getRequiredElement(".js-tick-strip"),
    unsupportedNote: getRequiredElement(".js-unsupported-note"),
    volumeRange: getRequiredElement(".js-volume-range"),
    volumeValue: getRequiredElement(".js-volume-value")
};
const state = {
    activeTab: null,
...
function isDefaultAudioState(audioState) {
    return (audioState.volume === DEFAULT_AUDIO_STATE.volume &&
...
function appendTicks(startVolume, endVolume, step, settings) {
    ...
    for (let volume = startVolume + step * direction; direction > 0 ? volume < endVolume : volume > endVolume; volume += step * direction) {
...
function renderTicks() {
    ...
    appendTicks(state.settings.minVolume, SETTINGS_LIMITS.normalVolume, state.settings.stepBelow100, state.settings);
    appendTicks(SETTINGS_LIMITS.normalVolume, state.settings.maxVolume, state.settings.stepAbove100, state.settings);
}
...
function renderAudibleTabs(tabs) {
    ...
    tabs.forEach((tab) => {
        ...
        const button = getRequiredElement(".tab-chip", fragment);
        const icon = getRequiredElement(".tab-chip__icon", fragment);
        const site = getRequiredElement(".tab-chip__site", fragment);
        const status = getRequiredElement(".tab-chip__status", fragment);
...
async function applyState(nextState) {
    ...
    const normalizedState = {
        volume: snapVolumeToStep(clampVolume(nextState.volume, state.settings.maxVolume, state.settings.minVolume), state.settings),
...
async function loadPopup() {
    const response = await sendMessage({ type: MESSAGE_TYPES.POPUP_GET_STATE });
...
function bindEvents() {
    ...
    elements.volumeRange.addEventListener("input", (event) => {
        ...
        void applyState({
            ...state.currentState,
            volume: snapVolumeToStep(sliderValueToVolume(Number(event.currentTarget.value), state.settings), state.settings)
